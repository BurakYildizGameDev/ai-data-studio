...
| `--engine parametric` | `ParametricEngine` | Compiles the contract directly into vectors. No code generation, no sandbox, ~2 ms. Declared correlations are enforced for numeric pairs *and* for a binary target driven by a numeric column (`is_fraud`, `churned`) via a latent-normal threshold that preserves the class ratio; binary-to-binary pairs are not yet correlated. **Single-table only** |
...
Enrichment applies to the **root table** in relational runs, the same rule as fraud
injection. `--engine parametric` rejects `--relational` outright: the parametric engine
draws columns from independent distributions and cannot maintain foreign-key
consistency, so it fails loudly instead of emitting orphans.
